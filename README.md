...
Retention counts only what could actually _serve_. A directory this process may not activate is not a fallback, so it is not a keeper — and it is not deleted either, because the library did not put it into that state and removing an install it cannot vouch for is not its call. So under `Untrusted` or `InstallWithoutCustody`, where nothing a previous process installed is activatable, `Retain` does not bound those directories and the tree grows by one per upgrade until something outside collects them. Under a clean verdict with neither flag set, `Retain` bounds it.
