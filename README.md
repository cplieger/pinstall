...
| `ArtifactDir` | Where the artifacts land: relative to the installer's private home when `Installer` is set, else to the extracted tree. Must resolve inside that tree |
...
| `LinkDir` | A directory under `Root` for the convenience symlink. Empty publishes none. One that resolves outside `Root` publishes none either |
| `Retain` | Predecessors kept besides the active version. Zero uses 1. Bounds the tree only under a clean verdict; see below |
...
Retention counts only what could actually _serve_. A directory this process may not activate is not a fallback, so it is not a keeper. It is not deleted either, because the library did not put it into that state and removing an install it cannot vouch for is not its call. So under `Untrusted` or `InstallWithoutCustody`, where nothing a previous process installed is activatable, `Retain` does not bound those directories and the tree grows by one per upgrade until something outside collects them. Under a clean verdict with neither flag set, `Retain` bounds it.
...
`ParseIdentities(raw string) (ids []int, rejected int)` turns a comma-separated list of numeric identities into the shape `TrustedUIDs` and `TrustedGIDs` take.
...
- **A version directory is complete or it does not exist.** Artifacts are written into a staging tree, each one is synced, a `.complete` sentinel naming the version is written and synced _last_, the directory is synced, and only then is it renamed into place. An interrupted install is detectable by the absence of the sentinel and is never a selection candidate.
- **Custody of the install root is a precondition, checked once and never repaired.** Every guarantee here reduces to one claim: the artifact activated came out of an archive matching the pinned digest. The digest is checked once, on bytes in flight, and from then on the claim rests entirely on nobody else being able to write into the tree. So the library verifies that before it fetches anything: the root and every directory above it, on the path as written, on the path it resolves to, and on the directory holding every symlink in between (a middle hop's directory repoints the chain just as the first one's does), must be owned by you or root and writable by nobody else; the version directory and every entry in it get the same questions at publish and at activation, because a file's mode is independent of its directory's. It _refuses_ rather than fixing what it finds. Two exemptions are deliberate and worth knowing before you rely on a clean verdict: a _sticky_ ancestor others can write passes, because sticky stops another user renaming your directory away (that is what makes `/tmp` usable as a root), though an ACL granting somebody `DELETE_CHILD`, the ownership of that directory or the right to rewrite its list does not pass, since each of those retires the sticky rule the exemption rests on; and group 0 is trusted, so a host whose root _group_ has members is outside what this checks. "Others can write" is read from the ACL as well as the mode, so a `1755` directory carrying an `EVERYONE@` write entry counts as shared.
- **Access-control lists are read, not guessed at.** A POSIX mode is a lossy projection of an ACL, and the loss runs the unsafe way: a directory reading `0755 root:root` can carry an entry granting a named user full write. So when a path carries one, it is parsed: POSIX.1e (`system.posix_acl_access`) and NFSv4 as OpenZFS serves it (`system.nfs4_acl_xdr`). The identities it actually grants write to are what get judged. That turns "there is an ACL here, I decline" into "uid 3000 can write this", which is a sentence an operator can act on. `TrustedUIDs` and `TrustedGIDs` are how you answer it: an identity that is already privileged on the host, an administrator reaching the volume over NFS who holds root anyway, gains nothing by writing these files, and saying so keeps the check enforcing everywhere else. `InstallWithoutCustody` remains for a volume with nothing precise to say about it: one whose filesystem does not make the mode its access decision at all. A list that cannot be read is a refusal, not an absence: a malformed one, a dialect field this parser does not recognise, a value longer than any list it would accept, the NFS client's own `system.nfs4_acl` (whose string principals it does not decode, so it is refused by name rather than misread), or a `getxattr` a sandbox denied all fail closed, because "I could not look" must never come back as "there was nothing there".
