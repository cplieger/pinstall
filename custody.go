...
func verifyCustody(dir string, trust trustedWriters) error {
	...
	// could swap it.
	if err := walkChain(dir, euid, trust); err != nil {
		return err
	}
	...
	if resolved != dir {
		return walkChain(resolved, euid, trust)
	}
	return nil
}
...
func walkChain(path string, euid int, trust trustedWriters) error {
	components := ancestors(path)
	parentSticky := false
	for i, component := range components {
		if err := checkComponent(component, euid, i == len(components)-1, parentSticky, trust); err != nil {
			return err
		}
...
func checkComponent(path string, euid int, leaf, parentSticky bool, trust trustedWriters) error {
	...
	if mode&os.ModeSymlink != 0 {
		return checkSymlink(path, owner, euid, parentSticky, trust)
	}
	...
	// A world-writable sticky ANCESTOR is not a boundary: sticky restricts removal and
	...
	// Linux system) a usable place to root an installation.
	//
	...
	if stranger, found := trust.firstStranger(writers, euid); found {
		return fmt.Errorf("%w: %s (mode %#o) can be modified by %s%s%s",
			ErrNoCustody, path, mode.Perm(), stranger, stickyNote(mode, leaf), trust.hint())
	}
	return nil
}

// stickyNote explains why the sticky bit did not save the installation root, for the one
// error where an operator is most likely to think it should have.
func stickyNote(mode os.FileMode, leaf bool) string {
	if leaf && mode&os.ModeSticky != 0 {
		return ", and the sticky bit stops another principal renaming this directory away but not creating a version directory inside it"
	}
	return ""
}
...
//
// The exception is a world-writable sticky parent, where sticky's rule is precisely "only
...
// question.
func checkSymlink(path string, owner, euid int, parentSticky bool, trust trustedWriters) error {
	if !parentSticky {
		return nil
	}
	if !trust.allowsOwner(owner, euid) {
		return fmt.Errorf("%w: %s is a symlink owned by uid %d inside a world-writable sticky directory, so that user can repoint it at a tree of their choosing%s",
...
func (m *Manager) checkCustody() {
	err := verifyCustody(nearestExisting(m.versionsDir), m.trust)
	if err != nil {
